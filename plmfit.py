...
########### Initializing the task specific head
    ...
    if args.head == 'mlp' :
        ...
        #config = utils.load_head_config(args.head_config)
       # assert args.head == config["network_type"], f'The loaded configuration ("{config["network_type"]}") does not match the specified architecture "{args.head}". Make sure to upload a configuration with "network_type":"{args.head}"'
        head = LinearRegression(embs.shape[1] , 1)#MLP(config['input_len'], config['hidden_len'], config['output_len'], config['activation_function'], config['dropout'])
...
########### Prepare data loaders
    ...
    test_dataset = data_utils.TensorDataset( embs_test  , torch.tensor(data_test['score'].values))  
...
#----------------------- Logging parameters ---------------------------------#
    ...
    if args.optimizer == 'adam':
        optimizer = torch.optim.Adam(ft_model.parameters(), lr=args.lr , betas=(0.9, 0.95))
    else:
    ...
    for epoch in range(args.epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for itr , trainig_data in enumerate(dataloader , 0):
                ...
                loss = criterion(torch.squeeze(outputs).float(), training_labels.float())  
                if phase == 'train':
                       loss.backward()
                       optimizer.step()
