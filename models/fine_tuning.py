       ...
       # model.head = model.head.to(device)
        ...
        if optimizer == 'adam':
            optim = torch.optim.Adam(
                model.parameters(), lr=self.lr, betas=(0.9, 0.99), weight_decay=0.1)
        ...
        for epoch in range(self.epochs):
            ...
            for phase in ['train', 'val']:
                ...
                for itr, trainig_data in enumerate(dataloaders_dict[phase], 0):
                    ...
                    loss = loss_f(torch.squeeze(outputs).float(),
                ...
                logger.log('({}) Loss: {:.4f} {:.4f}s'.format(
                    phase, epoch_loss, time.now() - epoch_start_time))

            # TODO: Implement early stopping
            if epoch_val_loss[-1] < best_val_loss:
                best_val_loss = epoch_val_loss[-1]
                torch.save(model.state_dict(
                ), f'./models/saved_models/model:{model.name}_head:{model.head_name}_ft:{self.method}.pt')
                best_epoch = epoch
