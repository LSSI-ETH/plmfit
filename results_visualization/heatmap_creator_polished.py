...
def collect_metrics(json_files=None, csv_file=None, data_type='aav', task_type='regression', method_type='feature_extraction', use_mlp=False):
    ...
    if json_files is not None:
        ...
        for file_path in json_files:
            with open(file_path, 'r') as file:
                ...
                if model_name is None or layer is None or reduction is None or head_type is None:
                    continue
    ...
    data_type_string = data_type_string.replace("many", "rest")
    ...
    main_title = f"{data_type_string.upper()} - {task_type.replace('_', ' ').upper()} Task | {method_type.replace('Feature Extraction', 'Feature Extraction (MLP)' if use_mlp else  'Feature Extraction (Linear)')}"
    ...
    plt.ylabel('Layer + Reduction', fontsize=13)  # Explicit y-axis label
