...
class FineTuner(ABC):
    ...
    def initialize_optimizer(self, model_parameters):
        ...
        if self.optimizer == 'sgd':
            ...
            return torch.optim.RMSprop(model_parameters, lr=self.lr, weight_decay=self.weight_decay)    
    ...
    def train(self, model, dataloaders_dict, log_interval = -1, on_ray_tuning=False, use_scheduler = False):
        ...
        if self.scheduler: 
            scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
                optimizer, 'min', factor = self.scheduler_factor, patience = self.scheduler_patience, eps = 1e-6)
            epoch_lrs = []
            ...
            for phase in ['train', 'val']:
                ...
                if phase == 'train':
                    ...
                    if epoch_loss < best_val_loss - 0.00001:
                        ...
                        epochs_no_improve += 1
                    self.logger.log(f'Current epochs without improvement: {epochs_no_improve}')
