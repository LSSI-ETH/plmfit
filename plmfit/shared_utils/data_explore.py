...
def evaluate_predictions(y_pred,y_test,c_type,n_class = 1):
    ...
    best_tre = get_threshold_MCC(y_pred,y_test,c_type)
    ...
    cm = functional.confusion_matrix(y_pred,y_test,"multilabel", threshold = best_tre, num_labels = 3)
    cm_fig = plot_confusion_matrices(cm,["Non-bind","Bind"],titles = ["Mouse","Cattle","Bat"])
    figures["con_mat"] = cm_fig

    # Plot ROC curve
    fpr, tpr, thresholds = functional.roc(y_pred,y_test,"multilabel", num_labels = 3)
    auc = functional.auroc(y_pred,y_test,"multilabel", num_labels = 3,average = 'none')
    roc_fig= plot_multilabel_ROC(fpr, tpr, auc)
    figures["ROC"] = roc_fig

    # Plot Precision Recall curve
    precision, recall, thresholds = functional.precision_recall_curve(y_pred,y_test,"multilabel", num_labels = 3)
