...
class ServerLogger(Logger):
    def __init__(self, experiment_name: str, base_dir='./loggers', server_url="https://log.stamko.info/upload.php", token='', server_path=''): 
        self.server_url = server_url
        self.token = token
        self.server_path = server_path
        self.last_post_time = None  # Track the last time a post was made
        super().__init__(experiment_name, base_dir)

    def post_to_server(self, file_path, data_name, file_type='file'):
        ...
        try:
            ...
            if response.status_code == 200:
                self.log(f'Successfully posted {data_name} to server.', to_server=False)  # Prevent recursive logging
                self.last_post_time = datetime.datetime.now()  # Update the last post time
            else:
                self.log(f'Failed to post {data_name} to server. Status code: {response.status_code}', to_server=False)
        except Exception as e:
            self.log(f'Exception occurred while posting to server: {e}', to_server=False)

    def log(self, text: str, to_server=True, force_send=False):
        super().log(text)  # Call the base class log method to save the log locally
        current_time = datetime.datetime.now()

        # Post to the server if 5 minutes have passed since the last log was successfully posted
        if to_server and (self.last_post_time is None or (current_time - self.last_post_time).total_seconds() > 300) or force_send:
            self.post_to_server(os.path.join(self.base_dir, self.file_name), self.file_name, file_type='log')

    
    def save_log_to_server(self, to_server=True):
        if to_server:
            self.post_to_server(os.path.join(self.base_dir, self.file_name), self.file_name)

    def save_plot(self, plot, plot_name, to_server=True):
        plot_path = os.path.join(self.base_dir, f"{self.experiment_name}_{plot_name}.png")
        plot.savefig(plot_path)
        plt.close(plot)  # Close the plot to free memory
        self.log(f'Saved plot with name "{plot_name}.png"')
    ...
    def save_data(self, data, data_name, to_server=True):
        file_path = os.path.join(self.base_dir, f"{self.experiment_name}_data.json")
        ...
        if to_server:
            try:
                self.post_to_server(file_path, f"{self.experiment_name}_data.json", file_type='json')
            except Exception as e:
                self.log(f'Error posting data to server: {e}')