...
def fine_tune(args, logger):
    ...
    #logger.log(data_loaders['test'].dataset.tensors[1].shape)
    ...
    trainer = Trainer(
        ...
        strategy=strategy,
        precision="16-mixed",
        callbacks=[model.early_stopping(), model.lr_monitor()]
    )
    ...
    if torch.cuda.is_available(): best_state_dict = convert_zero_checkpoint_to_fp32_state_dict(f'{logger.base_dir}/lightning_logs/best_model.ckpt', f'{logger.base_dir}/best_model.ckpt')
