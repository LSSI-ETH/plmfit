...
do
  ...
  sbatch --output="$experiment_dir/euler_output.out" --error="$experiment_dir/euler_error.err" --mem-per-cpu="$mem_per_cpu" --gpus="$gpus" --gres="gpumem:$gres" scripts/extract_embeddings_mass.sh "$function" "$data_type" "$plm" "$reduction" "$layer" "$output_dir" "$experiment_dir" "$experiment_name"
done
