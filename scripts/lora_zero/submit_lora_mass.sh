...
csv_file="./scripts/lora_zero/experiments_setup.csv"
...
do
  ...
  experiment_dir="$output_dir/$function/${ft_method}_${target_layers}_zero/$experiment_name/$uid"
  ...
  sbatch --job-name="lora_${data_type}_${plm}_${task}" \
         ...
         --time=$run_time:00:00 \
         scripts/lora_zero/lora_mass.sh \
